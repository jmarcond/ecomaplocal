...
class Registration {
	 ...
	 * checks all error possibilities, and creates a new user in the database if
	 * everything is fine
	 */
	private function registerNewUser() {
		if (empty ( $_POST ['user_name'] )) {
			...
			if (! $this->db_connection->connect_errno) {
				...
				$query_check_user_name = $this->db_connection->query ( "SELECT * FROM ".DB_USERS_TABLE." WHERE user_name = '" . $this->user_name . "';" );
				
				if ($query_check_user_name->num_rows == 1) {
					$this->errors [] = "Sorry, that user name is already taken. Please choose another one.";
				} else {
					// write new users data into database
					$query_new_user_insert = $this->db_connection->query ( "INSERT INTO ".DB_USERS_TABLE." (user_name, user_password_hash, user_email) VALUES('" . $this->user_name . "', '" . $this->user_password_hash . "', '" . $this->user_email . "');" );
					
					if ($query_new_user_insert) {
						$this->messages [] = "Your account has been created successfully. You can now log in.";
						$this->registration_successful = true;
						
						$checkid= $this->db_connection->query ( "SELECT user_id, user_name, user_email, user_password_hash FROM ".DB_USERS_TABLE." WHERE user_name = '" . $this->user_name . "';" );
						
						// if this user exists
						if ($checkid->num_rows == 1) {
							
							// get result row (as an object)
							$result_row = $checkid->fetch_object ();
